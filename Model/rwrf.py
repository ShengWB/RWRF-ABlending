...
class RWRandomForest:
    def __init__(
        self, n_trees, max_depth, n_feats, classifier=False, criterion="mse"
    ):
        """
    ...
    def predict(self, X,Y):
        ...
        return self._vote(tree_preds,Y)

    def _vote(self,predictions,Y):
        """
        ...
        if self.classifier:
            ...
            prediction=abs(predictions-Y)
            print(prediction)
            fangcha=np.var(prediction,axis=1)
            fangcha_sum=np.sum(fangcha)
            weight=fangcha/fangcha_sum
            predictions=predictions.T
            # weight=weight.T
            out=predictions*weight
            out=np.sum(out,axis=1)
            #out=[np.mean(x) for x in predictions.T]
            # out=[np.var(x) for x in predictions.T]
            # out=np.array(out)
            # out_sum=out.sum(axis=0)

            #out = [np.var(x)/sum((np.var(x))*x) for x in predictions.T]
        return np.array(out)